...
class AIHandler:
    ...
    def analyze_style(self, tweets: List[str]) -> str:
        # For analysis, we can keep using prompt as is or separate it too.
        # Since 'tweets' comes from user/API, separating is better practice but less critical for injection
        # as the output is just a profile description, not executed code.
        # However, malicious tweets could try to influence the profile to contain instructions.
        # Let's keep it simple for now as the plan focused on generation.
    ...
    def generate_tweet(self, topic: str, count: int = 1) -> List[str]:
        ...
        response = self._call_model(user_message, system_instruction=system_instruction)
    ...
    def generate_retweet_comment(self, original_tweet_text: str) -> str:
        ...
        return self._call_model(user_message, system_instruction=system_instruction).strip()
    ...
    def generate_tweet_from_image(self, image_path: str, count: int = 1) -> List[str]:
        ...
        try:
            ...
            response = self._call_model(user_message, images=[img], system_instruction=system_instruction)
    ...
    def _call_model(self, prompt: str, images: list = None, system_instruction: str = None) -> str:
        try:
            if self.provider == "gemini":
                # Pass system_instruction to GenerativeModel if available
